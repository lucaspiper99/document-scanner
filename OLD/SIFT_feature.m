...
%function SIFT_feature(img)
[img,map] = imread("DATASETS\InitialDataset\templates\template2_fewArucos.png");
        if size(img,3) == 1
            img = uint8(round(ind2rgb(img, map)*255));
        end
tic
row=size(img,1);
colum=size(img,2);
...
sigma0=sqrt(2);
octave=2;%6*sigma*k^(octave*level)<=min(m,n)/(2^(octave-2)) 3
level=3;
D=cell(1,octave);
for i=1:octave
